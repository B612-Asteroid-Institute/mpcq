...
def test_cross_match_observations_with_matches(
    ...
    np.testing.assert_allclose(
        ...
        final_results["separation_meters"].to_numpy(zero_copy_only=False)
...
def test_cross_match_query_uses_literal_bounds_and_projection(
    ...
    assert "CAST(obs.ra AS FLOAT64)" in query
    assert "CAST(obs.dec AS FLOAT64)" in query
    assert "input_bounds" not in query
    assert "SELECT DISTINCT stn" not in query
