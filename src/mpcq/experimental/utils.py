...
import quivr as qv
from adam_core.observations import SourceCatalog, calculate_observing_night
...
def reduce_deep_drilling_observations(
    ...
    observation_nights = (
        ...
        .append_column("mjd", observations.coordinates.time.mjd())
        .select(["id", "night", "mjd"])
    )
    ...
    for orbit in deep_drilling_orbits:
        ...
        for night in deep_drilling_nights:
            ...
            dds_night = DeepDrillingSummary.from_kwargs(
                ...
                mjd=mjds,
                keep=np.in1d(np.arange(len(obs_ids)), idx),
            )
