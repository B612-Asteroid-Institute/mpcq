...
class BigQueryMPCClient(MPCClient):
    ...
    def query_orbits(
        ...
        for col in selected_cols:
            if col in {"requested_provid", "primary_designation", "provid", "epoch"}:
                continue
            if col == "orbit_type":
                # MPC stores this as orbit_type_int; MPCOrbits exposes it as orbit_type.
                select_list.append("mpc_orbits.orbit_type_int AS orbit_type")
            elif col in {"mpc_orb_jsonb", "datastream_metadata"}:
    ...
    def query_submission_num_obs(self, submission_id: str) -> int:
        ...
        return cast(int, results["num_obs"].to_pylist()[0])
